---
title: Validation
description: Validate requests with type-safe schemas powered by Rust
---
...
app.post("/posts/:id", [completeSchema], ctx => {
...
app.post("/users", [userSchema], (ctx: UserContext) => {
  // TypeScript knows the exact types
  ctx.req.body.name;   // string
  ctx.req.body.email;  // string
  ctx.req.body.age;    // number | undefined
});
```
...
app.post("/users/batch", [batchSchema], ctx => {
  const { users } = ctx.req.body;
